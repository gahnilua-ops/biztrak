...
2. Write a tiny Python wrapper (see previous message) that forwards `{chat_id, user_id, text, mode}` to `https://<kai-host>/skill/telegram_brain` and sends `reply_text` back via `sendMessage`.
